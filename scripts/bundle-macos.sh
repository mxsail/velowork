...
# macOS App Bundle Script for Velowork
...
echo "==> Building Velowork for macOS"
echo "    Target: $TARGET"
...
BINARY_PATH="target/$TARGET/release/velowork"
if [[ ! -f "$BINARY_PATH" ]]; then
    # Try default release path (when built without --target)
    BINARY_PATH="target/release/velowork"
    if [[ ! -f "$BINARY_PATH" ]]; then
        echo "Error: Binary not found"
        echo "Checked: target/$TARGET/release/velowork"
        echo "Checked: target/release/velowork"
...
# Copy binary
echo "==> Copying binary..."
cp "$BINARY_PATH" "$MACOS_DIR/velowork"
chmod +x "$MACOS_DIR/velowork"
...
codesign --force --sign - "$MACOS_DIR/velowork"
...
if [[ "$CREATE_DMG" == true ]]; then
    echo "==> Creating DMG..."
    DMG_NAME="Velowork-$VERSION-$TARGET.dmg"
