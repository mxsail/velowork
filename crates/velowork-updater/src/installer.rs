...
pub fn restart_app(cx: &mut gpui::App) {
    if let Ok(exe) = std::env::current_exe() {
        ...
        match crate::process::command(&exe.to_string_lossy()).args(&args).spawn() {
            Ok(_) => {
                log::info!("[updater] Restarting velowork...");
                cx.quit();
            }
            Err(e) => log::error!("[updater] Failed to restart | error: {:#}", e),
        }
    }
}
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        if old_path.exists() {
            match std::fs::remove_file(&old_path) {
                Ok(()) => log::info!("[updater] Cleaned up old binary: {:?}", old_path),
                Err(e) => log::warn!("[updater] Failed to clean up old binary {:?} | error: {:#}", old_path, e),
            }
        }
    }
}
...
fn validate_binary(binary: &Path) -> Result<()> {
        ...
        match child.try_wait().context("failed to wait on validation process")? {
            ...
            None => {
                if start.elapsed() > timeout {
                    let _ = child.kill();
                    let _ = child.wait();
                    log::error!("[updater] Binary validation timed out, rolling back");
    ...
    if status.success() {
        log::info!("[updater] Binary validation passed");
        Ok(())
    } else {
        log::error!("[updater] Binary validation failed (exit {}), rolling back", status);
        let _ = std::fs::remove_file(binary);
        let _ = std::fs::rename(&old_path, binary);
...
fn find_binary(dir: &Path) -> Result<PathBuf> {
    #[cfg(unix)]
    let binary_name = "velowork";
    #[cfg(windows)]
    let binary_name = "velowork.exe";
...
fn replace_binary(current: &Path, new_binary: &Path) -> Result<()> {
    ...
    if let Err(e) = std::fs::copy(new_binary, &target) {
        log::error!("[updater] Failed to copy new binary, rolling back | error: {:#}", e);
    ...
    #[cfg(unix)]
    {
        ...
        if let Err(e) = std::fs::set_permissions(&target, std::fs::Permissions::from_mode(0o755)) {
            log::error!("[updater] Failed to set permissions, rolling back | error: {:#}", e);
    ...
    log::info!("[updater] Replaced binary at {:?}", target);
    Ok(())
}
