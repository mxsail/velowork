...
impl BatchedTextRun {
    ...
    pub fn paint(
        ...
        // For 2-column wide characters, pass cell_width * 2.0 so GPUI advances by 2 cells
        let char_width = if self.is_wide { cell_width * 2.0 } else { cell_width };

        // Shape and paint entire run at once, passing char_width for fixed-width spacing
        let _ = window
            .text_system()
            .shape_line(
...
pub(crate) fn is_default_bg(color: &Color, palette_bg: u32) -> bool {
    match color {
        Color::Named(NamedColor::Background) => true,
        Color::Indexed(idx) if *idx == 0 => false, // Black is not default bg
        Color::Spec(rgb_color) => {
            // Check if it matches the palette's terminal background
            let bg_r = ((palette_bg >> 16) & 0xFF) as u8;
            let bg_g = ((palette_bg >> 8) & 0xFF) as u8;
            let bg_b = (palette_bg & 0xFF) as u8;
