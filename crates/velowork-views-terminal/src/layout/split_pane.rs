...
use velowork_workspace::state::{SplitDirection, WindowId, Workspace};
use gpui::*;
...
pub enum DragState {
    ...
    Sidebar,
    /// Resizing right sidebar width
    RightSidebar,
}

/// Trait object wrapper for ActionDispatch in DragState (needs Clone).
pub trait ActionDispatchClone: Send + Sync {
    fn dispatch_action(&self, action: velowork_core::api::ActionRequest, cx: &mut gpui::App);
    ...
    match drag_state {
        DragState::Split { project_id, layout_path, left_child, right_child, direction, container_bounds, initial_mouse_pos, initial_sizes, visible_sizes_sum, action_dispatcher } => {
            ...
            if let Some(dispatcher) = action_dispatcher {
                dispatcher.dispatch_action(velowork_core::api::ActionRequest::UpdateSplitSizes {
                    project_id,
                    path: layout_path,
                    sizes: new_sizes,
                }, cx);
        ...
        DragState::ProjectColumn { divider_index, project_ids, available_size, vertical, initial_mouse_pos, initial_widths, min_col_width } => {
            ...
            // The two columns share a fixed total width, so their sizes must stay
            // conservative (`left + right == total`). Clamping each side
            // independently to `min_width` lets the sum shrink below the total
            // when dragged past either limit, leaving a gap between the columns
            // (and the divider keeps following the cursor while both borders
            // stay pinned). Anchor `left` within `[min, total - min]` and derive
            // `right` from the remainder so the split never loses width.
            let total = left_initial + right_initial;
            let left_new = (left_initial + delta_percent)
                .max(min_width)
                .min(total - min_width);
            let right_new = total - left_new;
        ...
        DragState::Sidebar | DragState::RightSidebar => {
    ...
    ResizeHandle::new(
        ...
        move |mouse_pos, cx| {
            ...
            let (initial_sizes, visible_sizes_sum) = workspace.read(cx).project(&project_id).and_then(|p| {
                ...
                if let velowork_workspace::state::LayoutNode::Split { sizes, children, .. } = node {
...
/// Render the right sidebar resize divider
pub fn render_right_sidebar_divider(active_drag: &ActiveDrag, cx: &App) -> impl IntoElement {
    let t = theme(cx);
    let active_drag = active_drag.clone();
    ...
    ResizeHandle::new(
        ...
        t.border_active,
        move |_, _| {
            *active_drag.borrow_mut() = Some(DragState::RightSidebar);
        },
    )
}
