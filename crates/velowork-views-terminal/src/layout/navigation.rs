...
use velowork_workspace::state::WindowId;
...
impl PaneMap {
    ...
    /// Return all registered panes in the map
    pub fn all_panes(&self) -> &[PaneBounds] {
        &self.panes
    }
...
/// Single decision point consulted by `TerminalPane::render` before it
/// re-claims input focus.
///
/// Combines the authoritative focus *layer* (owned by `FocusManager`) with the
/// ground-truth physical focus (GPUI `FocusHandle`) so that the terminal only
/// reclaims focus when BOTH hold:
///
///   1. the `FocusManager` layer permits a terminal (`Terminal` or `None`), and
///   2. no foreign input (AI side panel, popup/overlay text field, sidebar
///      input, …) currently holds the physical focus.
///
/// This is what stops the terminal's frequent timer-driven re-renders from
/// hijacking focus away from other components. It is the one place where the
/// question "may a terminal take focus now?" is answered — components never
/// call `window.focus(...)` on their own to steal focus from another category.
///
/// The physical-focus half lives here (in the terminal crate) because `PaneMap`
/// — the registry of every terminal pane's `FocusHandle` — is defined in this
/// crate; the layer half lives on `FocusManager` (in `velowork-workspace`).
pub fn terminal_may_claim_focus(
    fm: &FocusManager,
    _window_id: WindowId,
    _window: &Window,
    _cx: &App,
) -> bool {
    // The unified FocusManager authorizes a terminal to own input if the active layer is Terminal or None,
    // and no modal is active.
    fm.terminal_layer_allows_claim() && !fm.is_modal()
}
...
mod tests {
    ...
    fn register_keeps_same_pane_key_separate_per_window() {
        let mut map = PaneMap::new();
        let extra = WindowId::Extra(velowork_workspace::state::WindowState::default().id);
