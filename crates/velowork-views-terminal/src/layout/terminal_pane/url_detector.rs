...
impl UrlDetector {
    ...
    pub fn open_url(url: &str) {
        log::debug!("[views:terminal:url] Opening URL: {}", url);
    ...
    pub fn open_file(path: &str, file_line: Option<u32>, file_col: Option<u32>, opener: &str) {
        ...
        log::debug!(
            "[views:terminal:url] Opening file: {} (line: {:?}, col: {:?}, opener: {:?})",
            clean_path,
            file_line,
            file_col,
            opener
        ...
        if opener.is_empty() {
            ...
            #[cfg(target_os = "linux")]
            {
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command("xdg-open").arg(clean_path),
                );
            }
            #[cfg(target_os = "macos")]
            {
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command("open").arg(clean_path),
                );
            }
            #[cfg(target_os = "windows")]
            {
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command("cmd").args(["/C", "start", "", clean_path]),
                );
            }
            return;
        }
        ...
        match opener {
            "code" | "cursor" => {
                ...
                args.push(loc);
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command(opener).args(&args),
                );
            }
            "zed" => {
                // Zed: file:line
                ...
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command("zed").arg(&loc),
                );
            }
            "subl" | "sublime" => {
                ...
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command("subl").arg(&loc),
                );
            }
            "vim" | "nvim" => {
                // vim/nvim: +line file
            ...
            _ => {
                ...
                let _ = velowork_core::process::spawn_and_reap(
                    velowork_core::process::command(opener).arg(&loc),
                );
            }
        }
    }
