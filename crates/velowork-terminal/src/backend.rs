...
use crate::pty_manager::{PtyManager, SshClient};
...
pub trait TerminalBackend: Send + Sync {
    ...
    /// Get the active SSH session handle if this backend supports programmatic SSH
    fn get_ssh_session(&self, _terminal_id: &str) -> Option<Arc<russh::client::Handle<SshClient>>> {
        None
    }
    /// Get the business session id a terminal is connected to, if it is an SSH
    /// terminal. Returns `None` for local terminals or backends without SSH
    /// support. Used by UI code to look up per-session settings (e.g.
    /// `enable_sftp`) for the focused terminal.
    fn get_ssh_session_id(&self, _terminal_id: &str) -> Option<String> {
        None
    }
}
...
impl TerminalBackend for LocalBackend {
    ...
    fn get_ssh_session(&self, terminal_id: &str) -> Option<Arc<russh::client::Handle<SshClient>>> {
        self.pty_manager.get_ssh_session(terminal_id)
    }

    fn get_ssh_session_id(&self, terminal_id: &str) -> Option<String> {
        self.pty_manager.get_ssh_session_id(terminal_id)
    }
}
