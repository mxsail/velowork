...
impl Terminal {
    ...
    /// Push the active terminal palette so the event listener can answer
    ...
    pub fn set_palette(&self, palette: velowork_core::theme::TerminalPalette) {
    ...
    /// `OSC 7 ; file://host/path`, or fallback to inspecting local shell process cwd.
    /// Returns `None` until the shell has emitted at least one such sequence or local PID is resolved.
    pub fn reported_cwd(&self) -> Option<String> {
        if let Some(cwd) = self.reported_cwd.lock().clone() {
            return Some(cwd);
        }
        #[cfg(target_os = "linux")]
        if let Some(pid) = *self.shell_pid.lock() {
        ...
        #[cfg(target_os = "macos")]
        if let Some(pid) = *self.shell_pid.lock() {
            if let Ok(path) = libproc::libproc::proc_pid::pidcwd(pid as i32) {
