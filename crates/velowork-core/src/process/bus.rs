...
//! long-lived bidirectional streams owned by `velowork-terminal`. PTY headroom is
...
impl Lane {
    fn workers(self) -> usize {
        match self {
            // Sum across lanes (2 + 2 + 1 = 5) is the effective global cap on
...
impl CommandSpec {
    ...
    /// (+ cwd). Lets a review of `velowork::cmd` see exactly what ran, with which
        ...
        for lane in [Lane::Interactive, Lane::Poll, Lane::Long] {
            ...
            for n in 0..lane.workers() {
                ...
                if let Err(e) = std::thread::Builder::new()
                    .name(format!("velowork-cmd-{}-{n}", lane.name()))
                    .stack_size(256 * 1024)
...
fn run_job(spec: &CommandSpec, ctl: &Arc<JobControl>) -> std::io::Result<Output> {
    ...
    log::trace!(target: "velowork::cmd", "[{}] start {}", spec.lane.name(), detail);
    ...
    .unwrap_or_else(|panic| {
        ...
        log::error!(target: "velowork::cmd", "[{}] {} panicked: {msg}", spec.lane.name(), detail);
...
fn spawn_pipe_reader<R: std::io::Read + Send + 'static>(
    ...
    std::thread::Builder::new()
        .name("velowork-pipe-reader".into())
        .stack_size(128 * 1024)
        .spawn(move || {
            let mut buf = Vec::new();
        ...
        .unwrap_or_else(|e| panic!("failed to spawn pipe reader thread: {e}"))
}
