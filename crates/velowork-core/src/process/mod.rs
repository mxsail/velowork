...
pub fn open_in_file_manager(path: &std::path::Path) {
    ...
    #[cfg(windows)]
    let result = spawn_and_reap(command("explorer").arg(path_str.as_ref()));
    #[cfg(not(any(target_os = "linux", target_os = "macos", windows)))]
    let result: std::io::Result<()> = {
        log::warn!("open_in_file_manager is not supported on this platform: {path:?}");
        Ok(())
    };

    if let Err(e) = result {
        log::warn!("failed to open directory in file manager {path:?}: {e}");
    }
}
