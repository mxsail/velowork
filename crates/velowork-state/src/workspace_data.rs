...
impl WorkspaceData {
    /// Check if any project layout currently contains a terminal tab for `session_id`.
    pub fn has_session_tab(&self, session_id: &str) -> bool {
        self.projects
            .iter()
            .any(|p| p.layout.as_ref().is_some_and(|l| l.has_session_tab(session_id)))
    }
...
pub struct ProjectData {
    ...
    /// Project icon name, matching `assets/icons/{icon}.svg` (e.g. "monitor").
    /// Empty string means "use the default icon" in the UI. Converted to the
    /// `AppIcon` catalog enum at the UI boundary via `AppIcon::from_str`.
    #[serde(default)]
    pub icon: String,
    /// Free-form, multi-line project description entered in the add-project dialog.
    #[serde(default)]
