...
/// Default capture filter: everything at `info`, plus our own `velowork*` crates
/// (including the `velowork::cmd` command-bus target) at `debug`. Keeps the ring
/// free of `gpui`/`wgpu`/`winit` trace noise while still showing app debug;
/// the console can escalate specific targets (e.g. `velowork::cmd=trace`).
pub const DEFAULT_CAPTURE: &str = "info,velowork=debug";

/// Ring buffer capacity. At ~200 bytes/line this caps the console at ~200 KB.
...
impl LogHub {
    ...
    /// `info,velowork::cmd=trace`). Lock-free for concurrent loggers.
    pub fn set_capture_filter(&self, directives: &str) {
        ...
        log::info!(target: "velowork::log", "capture filter set to `{directives}`");
    }
...
struct VeloworkLogger {
    inner: env_logger::Logger,
    hub: &'static LogHub,
}

impl log::Log for VeloworkLogger {
...
pub fn init(inner: env_logger::Logger) {
    ...
    if log::set_boxed_logger(Box::new(VeloworkLogger { inner, hub })).is_err() {
        // A logger was already installed (e.g. a test harness). Leave it.
        eprintln!("velowork: a global logger was already set; in-app log capture disabled");
    }
}
...
mod tests {
    ...
    #[test]
    fn default_filter_captures_velowork_debug_drops_extern_debug() {
        let hub = LogHub::new(DEFAULT_CAPTURE);
        rec(&hub, log::Level::Debug, "velowork::cmd", "bus debug");
    ...
    fn runtime_filter_escalation_changes_what_is_captured() {
        ...
        rec(&hub, log::Level::Trace, "velowork::cmd", "before");
        assert!(hub.snapshot_since(0).is_empty());

        hub.set_capture_filter("velowork::cmd=trace");
        rec(&hub, log::Level::Trace, "velowork::cmd", "after");
        // The set_capture_filter info line is on target velowork::log, which the
        // `velowork::cmd=trace` directive does not admit, so only "after" lands.
    ...
    fn snapshot_since_returns_only_new_lines() {
        ...
        rec(&hub, log::Level::Info, "velowork", "a");
        rec(&hub, log::Level::Info, "velowork", "b");
        let cursor = hub.next_seq();
        rec(&hub, log::Level::Info, "velowork", "c");
    ...
    fn ring_caps_capacity_and_preserves_seq() {
        ...
        for i in 0..(RING_CAPACITY + 50) {
            rec(&hub, log::Level::Info, "velowork", &format!("line {i}"));
        }
        let lines = hub.snapshot_since(0);
    ...
    fn clear_empties_buffer_without_resetting_seq() {
        let hub = LogHub::new("trace");
        rec(&hub, log::Level::Info, "velowork", "x");
