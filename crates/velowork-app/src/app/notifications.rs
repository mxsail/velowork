...
//! those queues here via [`Velowork::process_terminal_notifications`] for every
...
pub(crate) fn show_notification(
    ...
    let _ = std::thread::Builder::new()
        ...
        builder.summary(&title).body(&body).appname("Velowork");
...
impl Velowork {
    ...
    pub(super) fn process_terminal_notifications(
        &mut self,
        dirty_terminal_ids: &std::collections::HashSet<String>,
        cx: &mut Context<Self>,
    ) {
        ...
        // Read the notification settings; bail if the feature is off.
        ...
        for (tid, osc, _bell) in drained {
            ...
            // OSC 9 carries no title; fall back to the project name.
            let fallback_title = resolved
                .as_ref()
                .map(|(_, name, _)| name.clone())
                .unwrap_or_else(|| "Velowork".to_string());
    ...
    /// and "no Velowork window focused" all return false, so they notify.
        ...
        focus_manager.update(cx, |fm, cx| {
            workspace.update(cx, |ws, cx| {
                if reveal {
                    ...
                    ws.set_focused_project(fm, Some(pid.clone()), target, cx);
                }
