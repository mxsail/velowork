...
//! (sidebar, focus, overlays). PRD ref: `plans/multi-window.md`'s "Velowork
...
//! to the per-window `FocusManager` of whichever Velowork window currently has
//! OS focus (PRD user story 27 / acceptance criterion 13). When no Velowork
...
/// be exercised without standing up the full `Velowork` entity (whose
...
/// or fall back to `WindowId::Main` if no Velowork window is focused (e.g. another
...
/// 13 (CLI fallback). Used by `Velowork::focus_manager_for_active_window` to
/// route remote-bridge actions (the existing `velowork action` CLI verb +
/// future `velowork open <path>`-style verbs) to the correct per-window
/// `FocusManager`.
#[allow(dead_code)]
...
impl Velowork {
    ...
    /// each entry not yet tracked in `Velowork.extra_windows`. Idempotent —
    ...
    /// Resolve the `(WindowId, Entity<FocusManager>)` of whichever Velowork
    /// window currently has OS focus, falling back to
    /// `(WindowId::Main, main_window.focus_manager())` if no Velowork window is
    ...
    /// register the resulting `Entity<WindowView>` in `Velowork.extra_windows`.
    ...
    fn open_extra_window(&mut self, window_id: WindowId, cx: &mut Context<Self>) {
        ...
        // `velowork-state`; aliased on import to disambiguate from the gpui
        ...
        let result = cx.open_window(
            WindowOptions {
                ...
                // transparent surface up front so rounded corners don't show
                // square tips on KDE Plasma (Wayland).
            ...
            move |window, cx| {
                ...
                let view_for_velowork = view.clone();
                let handle = window.window_handle();
                // Defer the registration: this build closure runs while Velowork
                // is already leased (the workspace observer that triggered
                // `open_extra_window` holds the Velowork update lease for the
                // duration of `flush_effects`). Calling `velowork.update` here
                ...
                let velowork_for_register = velowork.clone();
                cx.defer(move |cx| {
                    velowork_for_register.update(cx, |this, cx| {
                        this.extra_windows.insert(window_id, view_for_velowork.clone());
                        ...
                        cx.subscribe(&view_for_velowork, Velowork::handle_window_view_event).detach();
                    });
                });
                ...
                // `Velowork.extra_windows` + `extra_window_handles` (so the
                ...
                // `Velowork.extra_windows` keeps it alive until the second
                // step). The Velowork-side removes drop the strong handle so
...
mod tests {
    ...
    fn ignores_main_window_id() {
        // Main is addressed separately on Velowork (Velowork.main_window field), not
    ...
    fn default_workspace_yields_no_extras_to_open() {
        ...
        // `main.rs::cx.open_window` for the Velowork view, so the visible
    ...
    fn corrupt_workspace_json_errors_without_panic_and_fallback_has_no_extras() {
        ...
        // test in `crates/velowork-state/src/workspace_data.rs::tests` (line
