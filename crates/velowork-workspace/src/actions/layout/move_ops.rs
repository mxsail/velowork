...
impl Workspace {
    ...
    fn move_pane_same_project(
        ...
        let new_focus_path = {
            ...
            let is_parent_tabs = if !target_path.is_empty() {
                let parent_path = &target_path[..target_path.len() - 1];
                matches!(layout.get_at_path(parent_path), Some(LayoutNode::Tabs { .. }))
            } else {
                false
            };

            if zone == DropZone::Center && is_parent_tabs {
                let parent_path = &target_path[..target_path.len() - 1];
                let insert_idx = target_path.last().copied().map(|i| i + 1).unwrap_or(0);
                if let Some(LayoutNode::Tabs { children, active_tab }) = layout.get_at_path_mut(parent_path) {
                    let clamped = insert_idx.min(children.len());
    ...
    fn move_pane_cross_project(
        ...
        let new_focus_path = if let Some(ref mut tgt_layout) = tgt_project.layout {
            ...
            let is_parent_tabs = if !target_path.is_empty() {
                let parent_path = &target_path[..target_path.len() - 1];
                matches!(tgt_layout.get_at_path(parent_path), Some(LayoutNode::Tabs { .. }))
            } else {
                false
            };
            ...
            if zone == DropZone::Center && is_parent_tabs {
                let parent_path = &target_path[..target_path.len() - 1];
                let insert_idx = target_path.last().copied().map(|i| i + 1).unwrap_or(0);
                if let Some(LayoutNode::Tabs { children, active_tab }) = tgt_layout.get_at_path_mut(parent_path) {
                    let clamped = insert_idx.min(children.len());
                    children.insert(clamped, source_node);
                    *active_tab = clamped;
                }
