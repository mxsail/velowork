...
impl Workspace {
    ...
    pub fn split_terminal(
        ...
        log::debug!("[workspace:layout] split_terminal | project_id={} path={:?}", project_id, path);
        ...
        let new_path = if let Some(project) = self.project_mut(project_id) {
            if let Some(ref mut layout) = project.layout {
                if let Some(node) = layout.get_at_path_mut(&split_path) {
                    log::debug!("[workspace:layout] Splitting node at path {:?}", split_path);
                    let old_node = node.clone();
                    let inherited_shell = match &old_node {
                        LayoutNode::Terminal { shell_type, .. } => shell_type.clone(),
                        LayoutNode::Tabs { children, active_tab } => {
                            children.get(*active_tab).and_then(|c| match c {
                                LayoutNode::Terminal { shell_type, .. } => Some(shell_type.clone()),
                                _ => None,
                            }).unwrap_or(velowork_terminal::shell_config::ShellType::Default)
                        }
                        _ => velowork_terminal::shell_config::ShellType::Default,
                    };
                    let new_terminal_node = LayoutNode::Terminal {
                        terminal_id: None,
                        shell_type: inherited_shell,
                        minimized: false,
                        detached: false,
                        zoom_level: 1.0,
                    };
                    *node = LayoutNode::Split {
                        direction,
                        sizes: vec![50.0, 50.0],
                        children: vec![old_node, new_terminal_node],
                    };
                    layout.normalize();
                    log::debug!("[workspace:layout] Split complete");
