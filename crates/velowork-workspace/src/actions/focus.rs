...
impl Workspace {
    ...
    pub fn set_focused_project(
        ...
        // 持久化当前活动项目，使重启后能恢复到正确的项目，
        // 否则 AI 助手聊天记录（按项目 id 保存）在重启后加载不到。
        // 放在最后，避免提前 move project_id。
        self.persist_focused_project_id(window_id, project_id, cx);
    ...
    pub fn set_focused_project_individual(
        ...
        // 持久化当前活动项目（individual 模式同样需要，确保重启后恢复正确）。
        // 放在最后，避免提前 move project_id。
        self.persist_focused_project_id(window_id, project_id, cx);
    ...
    pub fn set_fullscreen_terminal(
        ...
        log::debug!("[workspace:focus] set_fullscreen_terminal | project_id={} terminal_id={}", project_id, terminal_id);
        ...
        log::debug!("[workspace:focus] Terminal layout path: {:?}", layout_path);
        ...
        log::debug!("[workspace:focus] Fullscreen terminal entered | terminal_id={}", terminal_id);
