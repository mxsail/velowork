use anyhow::{Context, Result, bail};
use serde::Deserialize;
use std::collections::HashMap;
use std::path::Path;
use velowork_state::{SessionProtocol, SshAuthType};

use super::common::normalize_optional_string;
use super::{ImportContext, ImportedSession, SessionImporter};

#[derive(Debug, Deserialize)]
struct FinalShellFolder {
    id: String,
    name: String,
    parent_id: Option<String>,
    #[serde(default)]
    delete_time: u64,
}

#[derive(Debug, Deserialize)]
struct FinalShellConnection {
    #[serde(default)]
    name: String,
    #[serde(default)]
    host: String,
    #[serde(default)]
    port: Option<u64>,
    #[serde(default)]
    user_name: Option<String>,
    #[serde(default)]
    parent_id: Option<String>,
    #[serde(default)]
    conection_type: Option<i32>,
    #[serde(default)]
    description: Option<String>,
    #[serde(default)]
    delete_time: u64,
}

pub struct FinalShellImporter;

impl SessionImporter for FinalShellImporter {
    fn id(&self) -> &'static str {
        "finalshell"
    }

    fn display_name(&self) -> &'static str {
        "FinalShell (conn directory)"
    }

    fn file_extensions(&self) -> &'static [&'static str] {
        &[]
    }

    fn is_directory_importer(&self) -> bool {
        true
    }

    fn can_handle(&self, path: &Path) -> bool {
        if path.is_dir() {
            // Check if directory contains FinalShell indicators
            path.join("folder.json").is_file()
                || std::fs::read_dir(path)
                    .map(|mut r| {
                        r.any(|entry| {
                            entry
                                .map(|e| {
                                    e.file_name()
                                        .to_str()
                                        .map(|s| s.ends_with("_connect_config.json"))
                                        .unwrap_or(false)
                                })
                                .unwrap_or(false)
                        })
                    })
                    .unwrap_or(false)
        } else {
            false
        }
    }

    fn parse(&self, ctx: &ImportContext) -> Result<Vec<ImportedSession>> {
        let root = &ctx.source_path;
        if !root.is_dir() {
            bail!("FinalShell import source must be a directory (e.g. conn directory)");
        }

        let mut folders = HashMap::new();
        let mut connections = Vec::new();
        collect_finalshell_entries(root, &mut folders, &mut connections)?;

        if connections.is_empty() {
            bail!("No FinalShell *_connect_config.json connection files found in {}", root.display());
        }

        Ok(connections
            .into_iter()
            .filter_map(|conn| finalshell_session_from_connection(conn, &folders))
            .collect())
    }
}

fn collect_finalshell_entries(
    dir: &Path,
    folders: &mut HashMap<String, FinalShellFolder>,
    connections: &mut Vec<FinalShellConnection>,
) -> Result<()> {
    let entries = std::fs::read_dir(dir)
        .with_context(|| format!("Cannot read directory: {}", dir.display()))?;

    for entry in entries {
        let entry = entry?;
        let path = entry.path();
        if path.is_dir() {
            collect_finalshell_entries(&path, folders, connections)?;
            continue;
        }

        let Some(file_name) = path.file_name().and_then(|name| name.to_str()) else {
            continue;
        };

        if file_name == "folder.json" {
            if let Ok(folder) = read_finalshell_json::<FinalShellFolder>(&path) {
                if folder.delete_time == 0 {
                    folders.insert(folder.id.clone(), folder);
                }
            }
        } else if file_name.ends_with("_connect_config.json") {
            if let Ok(connection) = read_finalshell_json::<FinalShellConnection>(&path) {
                connections.push(connection);
            }
        }
    }
    Ok(())
}

fn read_finalshell_json<T: for<'de> Deserialize<'de>>(path: &Path) -> Result<T> {
    let content = std::fs::read_to_string(path)
        .with_context(|| format!("Cannot read FinalShell JSON file: {}", path.display()))?;
    serde_json::from_str(&content)
        .with_context(|| format!("Invalid FinalShell JSON: {}", path.display()))
}

fn finalshell_session_from_connection(
    conn: FinalShellConnection,
    folders: &HashMap<String, FinalShellFolder>,
) -> Option<ImportedSession> {
    // 100 indicates SSH in FinalShell
    if conn.delete_time != 0 || (conn.conection_type.is_some() && conn.conection_type != Some(100)) {
        return None;
    }

    let host = conn.host.trim().to_string();
    if host.is_empty() {
        return None;
    }

    let port = conn
        .port
        .and_then(|port| u16::try_from(port).ok())
        .filter(|port| *port > 0)
        .unwrap_or(22);

    let username = conn
        .user_name
        .as_deref()
        .map(str::trim)
        .filter(|username| !username.is_empty())
        .unwrap_or("root")
        .to_string();

    let name = if conn.name.trim().is_empty() {
        host.clone()
    } else {
        conn.name.trim().to_string()
    };

    let group_path = conn
        .parent_id
        .as_deref()
        .and_then(|parent_id| finalshell_group_path(parent_id, folders));

    Some(ImportedSession {
        name,
        protocol: SessionProtocol::Ssh,
        host,
        port,
        username,
        auth_type: SshAuthType::Password { password: None },
        group_path,
        description: normalize_optional_string(conn.description),
    })
}

fn finalshell_group_path(
    parent_id: &str,
    folders: &HashMap<String, FinalShellFolder>,
) -> Option<Vec<String>> {
    if parent_id.trim().is_empty() || parent_id == "root" || parent_id == "0" {
        return None;
    }

    let mut path = Vec::new();
    let mut current = parent_id;
    for _ in 0..folders.len() {
        let folder = folders.get(current)?;
        if !folder.name.trim().is_empty() {
            path.push(folder.name.clone());
        }
        let Some(parent_id) = folder.parent_id.as_deref() else {
            break;
        };
        if parent_id == "root" || parent_id == "0" || parent_id.trim().is_empty() {
            break;
        }
        current = parent_id;
    }

    path.reverse();
    if path.is_empty() {
        None
    } else {
        Some(path)
    }
}
