...
//! velowork-layout — Layout tree algorithms.
//!
...
pub use velowork_core::types::SplitDirection;
...
impl LayoutNode {
    ...
    /// Returns true if this layout subtree contains at least one active, visible real terminal
    /// (i.e. not minimized, not detached, and not a Welcome dashboard placeholder).
    pub fn has_real_terminal(&self) -> bool {
        match self {
            LayoutNode::Terminal { minimized, detached, shell_type, .. } => {
                !*minimized && !*detached && *shell_type != ShellType::Welcome
            }
            LayoutNode::Split { children, .. } => {
                children.iter().any(|c| c.has_real_terminal())
            }
            LayoutNode::Tabs { children, active_tab } => {
                children.get(*active_tab).map(|c| c.has_real_terminal()).unwrap_or(false)
            }
        }
    }

    /// Check if this layout node tree contains at least one terminal tab for the given `session_id`.
    ...
    /// Create a new terminal node with a specific shell type
    pub fn new_terminal_with_shell(shell_type: ShellType) -> Self {
        LayoutNode::Terminal {
            terminal_id: None,
            minimized: false,
            detached: false,
    ...
    /// Collect all minimized terminal IDs in this layout tree along with path and shell type
    pub fn collect_minimized_terminals(&self) -> Vec<(String, Vec<usize>, ShellType)> {
    ...
    fn collect_minimized_recursive(
        &self,
        result: &mut Vec<(String, Vec<usize>, ShellType)>,
        ...
        match self {
            LayoutNode::Terminal {
                ...
                if *minimized
                    && let Some(id) = terminal_id {
                        result.push((id.clone(), current_path, shell_type.clone()));
                    }
            }
            LayoutNode::Split { children, .. } | LayoutNode::Tabs { children, .. } => {
                for (i, child) in children.iter().enumerate() {
    ...
    /// Collect all detached terminal IDs in this layout tree along with path and shell type
    pub fn collect_detached_terminals(&self) -> Vec<(String, Vec<usize>, ShellType)> {
        let mut result = Vec::new();
    ...
    /// Returns true if this layout tree contains at least one `Split` node.
    ///
    /// A split means several terminal panes are shown simultaneously
    /// (side-by-side or stacked) — the UI's notion of "分屏". When there is no
    /// split the project renders as a single terminal pane regardless of how
    /// many terminals are stacked behind inactive tabs.
    pub fn has_split(&self) -> bool {
        match self {
            LayoutNode::Terminal { .. } => false,
            LayoutNode::Split { .. } => true,
            LayoutNode::Tabs { children, .. } => children.iter().any(|c| c.has_split()),
        }
    }

    /// Returns true if this layout tree contains at least one initialized
    /// (running) terminal — a `Terminal` node whose `terminal_id` is set.
    /// Minimized and detached terminals still count as running.
    pub fn has_running_terminals(&self) -> bool {
        match self {
            LayoutNode::Terminal { terminal_id, .. } => terminal_id.is_some(),
            LayoutNode::Split { children, .. } | LayoutNode::Tabs { children, .. } => {
                children.iter().any(|c| c.has_running_terminals())
            }
        }
    }
    ...
    /// Returns true if this node is a terminal that is not minimized and not detached,
    /// or a container containing at least one such terminal.
    pub fn is_visible(&self) -> bool {
        match self {
            LayoutNode::Terminal { minimized, detached, .. } => !*minimized && !*detached,
            LayoutNode::Split { children, .. } | LayoutNode::Tabs { children, .. } => {
                children.iter().any(|c| c.is_visible())
            }
        }
    }

    /// Find path to the first visible terminal (not minimized and not detached).
    pub fn find_first_visible_terminal_path(&self) -> Option<Vec<usize>> {
        self.find_first_visible_terminal_path_recursive(vec![])
    }

    fn find_first_visible_terminal_path_recursive(&self, current_path: Vec<usize>) -> Option<Vec<usize>> {
        match self {
            LayoutNode::Terminal { minimized, detached, .. } => {
            ...
            LayoutNode::Split { children, .. } => {
                for (i, child) in children.iter().enumerate() {
                    let mut child_path = current_path.clone();
                    child_path.push(i);
                    if let Some(path) = child.find_first_visible_terminal_path_recursive(child_path) {
                        return Some(path);
                    }
                }
                None
            ...
            LayoutNode::Tabs { children, active_tab, .. } => {
                ...
                if let Some(active_child) = children.get(active_idx) {
                    let mut child_path = current_path.clone();
                    child_path.push(active_idx);
                    if let Some(path) = active_child.find_first_visible_terminal_path_recursive(child_path) {
                        return Some(path);
                    }
                }
                for (i, child) in children.iter().enumerate() {
    ...
    /// Find the nearest visible terminal relative to `current_path`.
    /// 1. Tries immediate siblings in the same parent (Split or Tabs).
    /// 2. If parent has no visible siblings, walks up the tree.
    /// 3. Falls back to `find_first_visible_terminal_path()`.
    pub fn find_sibling_or_visible_terminal_path(&self, current_path: &[usize]) -> Option<Vec<usize>> {
        if current_path.is_empty() {
            return self.find_first_visible_terminal_path();
        }

        let Some(&current_index) = current_path.last() else {
            return self.find_first_visible_terminal_path();
        };
        let parent_path = &current_path[..current_path.len() - 1];

        if let Some(LayoutNode::Split { children, .. } | LayoutNode::Tabs { children, .. }) =
            self.get_at_path(parent_path)
        {
            // Try next sibling (i + 1, i + 2...)
            for (i, child) in children.iter().enumerate().skip(current_index + 1) {
            ...
            // Try previous sibling (i - 1, i - 2... 0)
            for (i, child) in children.iter().enumerate().take(current_index).rev() {
                let mut child_path = parent_path.to_vec();
                child_path.push(i);
                if let Some(path) = child.find_first_visible_terminal_path_recursive(child_path) {
                    return Some(path);
        ...
        if !parent_path.is_empty()
            && let Some(path) = self.find_sibling_or_visible_terminal_path(parent_path)
        {
            return Some(path);
        }

        self.find_first_visible_terminal_path()
    }
    ...
    #[allow(dead_code)]
    pub fn from_api(api: &velowork_core::api::ApiLayoutNode) -> Self {
        match api {
            velowork_core::api::ApiLayoutNode::Terminal {
                terminal_id,
                minimized,
                detached,
                ..
            ...
            velowork_core::api::ApiLayoutNode::Split {
                direction,
                sizes,
                children,
            } => LayoutNode::Split {
            ...
            velowork_core::api::ApiLayoutNode::Tabs {
                children,
                active_tab,
            } => LayoutNode::Tabs {
                children: children.iter().map(LayoutNode::from_api).collect(),
    ...
    pub fn from_api_prefixed(api: &velowork_core::api::ApiLayoutNode, prefix: &str) -> Self {
        match api {
            velowork_core::api::ApiLayoutNode::Terminal {
                terminal_id,
                minimized,
    ...
    pub fn to_api(&self) -> velowork_core::api::ApiLayoutNode {
    ...
    ) -> velowork_core::api::ApiLayoutNode {
        match self {
            LayoutNode::Terminal {
                terminal_id,
                minimized,
...
mod tests {
    ...
    #[test]
    fn test_has_session_tab() {
        use velowork_core::shell::ShellType;

        let ssh_terminal = LayoutNode::Terminal {
            terminal_id: Some("t1".to_string()),
            minimized: false,
            detached: false,
            shell_type: ShellType::Custom {
                path: "ssh".to_string(),
                args: vec!["--id".to_string(), "sess-1".to_string(), "root@host".to_string()],
            },
            zoom_level: 1.0,
        };

        let local_terminal = terminal("t2");
    ...
    #[test]
    fn test_new_terminal_with_shell() {
        use velowork_core::shell::ShellType;

        let ssh_shell = ShellType::Custom {
            path: "ssh".to_string(),
            args: vec!["--id".to_string(), "sess-123".to_string(), "user@remote".to_string()],
        };
        let node = LayoutNode::new_terminal_with_shell(ssh_shell.clone());
        match node {
            LayoutNode::Terminal { terminal_id, shell_type, .. } => {
                assert!(terminal_id.is_none());
                assert_eq!(shell_type, ssh_shell);
                assert!(shell_type.is_remote());
            }
    ...
    #[test]
    fn test_find_sibling_or_visible_terminal_path() {
        let t1 = terminal("t1");
        let t2 = terminal_minimized("t2");
        let t3 = terminal("t3");
        let root = hsplit(vec![t1, t2, t3]);

        // t1 is at [0], t2 is minimized at [1], t3 is at [2]
        assert!(root.is_visible());
        assert_eq!(root.find_first_visible_terminal_path(), Some(vec![0]));

        // From [0], sibling search should find [2] because [1] is minimized
        assert_eq!(root.find_sibling_or_visible_terminal_path(&[0]), Some(vec![2]));
        // From [1] (which is minimized), sibling search should find [2]
        assert_eq!(root.find_sibling_or_visible_terminal_path(&[1]), Some(vec![2]));
        // From [2], sibling search should find [0]
        assert_eq!(root.find_sibling_or_visible_terminal_path(&[2]), Some(vec![0]));
    }
}
