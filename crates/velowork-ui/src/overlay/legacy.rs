//! Backward-compatible overlay helpers.
//!
//! These were previously defined in `overlay.rs` at the crate root. They are
//! kept here (and re-exported from `crate::overlay`) so existing callers such as
//! `Workspace` / `WindowView` continue to compile during the migration to the
//! unified overlay framework. New code should prefer the framework in this
...
///
/// Implement this for your overlay's event enum to enable automatic close
/// handling.
...
impl<T: 'static + Render> OverlaySlot<T> {
    ...
    ///
    /// Returns the entity clone if open, None otherwise. Use with `.when()` and
    /// `.child()` in your render method.
...
/// modal on `is_close()`, and opens it via `open_modal` (which notifies).
#[macro_export]
